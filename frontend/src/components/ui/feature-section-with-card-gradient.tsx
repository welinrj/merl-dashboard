...
export interface FeatureCardGradientProps {
...
/** Reusable gradient shell used by live project cards and the registry demo. */
export function FeatureCardGradient({
  children,
  className = "",
  pattern,
  gridSize = 20,
}: FeatureCardGradientProps) {
  return (
    <article
      className={`relative overflow-hidden rounded-3xl bg-gradient-to-b from-neutral-100 to-white p-6 ${className}`.trim()}
    >
      <Grid pattern={pattern} size={gridSize} />
      {children}
    </article>
  );
}
    ...
    <div className="py-20 lg:py-40">
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-3 md:gap-2 lg:grid-cols-4">
        {features.map((feature) => (
          <FeatureCardGradient key={feature.title}>
...
export const Grid = ({
  ...
  return (
    <div
      className="feature-card-grid pointer-events-none absolute left-1/2 top-0 -ml-20 -mt-2 h-full w-full [mask-image:linear-gradient(white,transparent)]"
      aria-hidden="true"
    >
