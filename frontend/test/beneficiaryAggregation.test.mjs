...
test('sums all reported records without discarding repeated projects', () => {
  const result = aggregateBeneficiaries([
    { project_id:'a', total_direct:10, female:6, male:4, youth:3, double_counting_check:true },
    { project_id:'a', total_direct:20, female:11, male:9, persons_with_disability:2 },
    { project_id:'b', total_direct:0, female:0, male:0, indirect:5 },
  ]);
  assert.equal(result.total_direct,30);
