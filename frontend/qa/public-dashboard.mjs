...
check('legacy login route returns to the shared public dashboard',new URL(page.url()).hash==='#/dashboards');
check('redirected public page keeps one header login',await page.locator('.pbd-root form.pbd-header-login').count()===1);
await page.goto('http://localhost:5199/#/project-setup',{waitUntil:'domcontentloaded'});
await page.locator('.pbd-root .pbd-kpis').waitFor({timeout:15000});
check('anonymous private route returns to the public dashboard',new URL(page.url()).hash==='#/dashboards');
check('anonymous private route does not render the authenticated workspace',await page.locator('.dsh:not(.pbd-root)').count()===0);
