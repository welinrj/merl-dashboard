...
async function check(page, label) {
  const result = await page.locator('.dsh-head').evaluate(header => {
    ...
    return {
      ...
      menuClear:!menu || getComputedStyle(menu).display==='none' || !overlaps(g,rect(menu)),
      horizontalOverflow:document.documentElement.scrollWidth>window.innerWidth+2,
      headerWordmark:!!header.querySelector('.pbd-head-brand'),
    };
  });
  for (const [key,value] of Object.entries(result)) {
    const expected = key==='imageCount'?3:['horizontalOverflow','headerWordmark'].includes(key)?false:true;
    if (value!==expected) throw new Error(`${label}: ${key} expected ${expected}, got ${value}`);
  }
  const sidebar = page.locator('.dsh-side .dsh-brand-title');
  if ((await sidebar.textContent()).trim() !== 'MERL' && !(await sidebar.textContent()).includes('MERL')) throw new Error(`${label}: sidebar MERL identity missing`);
...
try {
  for (const authenticated of [false,true]) {
    ...
    for (const [width,height] of [[1440,900],[1024,768],[768,1024],[390,844],[320,720]]) {
      ...
      await check(page, `${authenticated?'Workspace':'Public'} ${width}px`);
      if (width<=760) {
