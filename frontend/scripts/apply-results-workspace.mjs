...
edit(root + 'pages/MerlReporting.jsx', (source) => {
  ...
  const saveButton = 'onClick={() => onSave(v)}';
  const last = s.lastIndexOf(saveButton);
  if (last < 0) throw new Error('Record save button not found');
  s = s.slice(0, last) + 'onClick={() => onSave(v, file)} disabled={busy}' + s.slice(last + saveButton.length);
  return s;
});
