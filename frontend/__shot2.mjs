...
const PROJECT = { id: 'p1', code: 'VCRP-001', name: 'Vanuatu Coastal Resilience Programme',
  status: 'at_risk', category: 'Climate Change Adaptation', project_type: 'Adaptation', donor: 'GCF',
  currency: 'VUV', budget_vuv: 480000000, spent_vuv: 210000000, start_date: '2025-01-01',
  end_date: '2027-12-31', provinces: ['SHEFA','TAFEA'], coverage_type: 'Multi-Province',
  lead_agency: 'DoCC', registration_status: 'approved' };
const TABLES = {
  v_projects: [PROJECT],
...
const shots = [['/dashboards',950,'overview'],['/analytics/results',1100,'results'],['/analytics/risks',1100,'risks'],
  ['/project-setup',1000,'project-setup'],['/merl-reporting',1000,'merl-reporting'],['/review',900,'review'],
  ['/reports',900,'reports'],['/admin',900,'admin']];
for (const lng of ['en','fr']) {
  const ctx = await b.newContext({ viewport: { width: 1440, height: 900 } });
  await ctx.addInitScript(([l, ref, exp]) => {
    localStorage.setItem('merl.lang', l);
    localStorage.setItem(`sb-${ref}-auth-token`, JSON.stringify({ access_token:'stub', token_type:'bearer',
      expires_in:999999, expires_at:exp, refresh_token:'stub',
      user:{ id:'u1', email:'meo@docc.gov.vu', aud:'authenticated', role:'authenticated' } }));
  }, [lng, REF, now + 999999]);
  await ctx.route(`${HOST}/**`, async (route) => {
    const p = new URL(route.request().url()).pathname;
    let body = [];
    if (p.endsWith('/rpc/current_profile')) body = [{ id:'u1', email:'meo@docc.gov.vu', full_name:'DoCC M&E Officer', role:'ROLE_DOCC_MEO' }];
    else if (p.startsWith('/auth/v1/user')) body = { id:'u1', email:'meo@docc.gov.vu', aud:'authenticated', role:'authenticated' };
    else if (p.startsWith('/rest/v1/')) body = TABLES[p.replace('/rest/v1/','').split('/')[0]] ?? [];
  ...
  for (const [path, h, name] of shots) {
    ...
    await page.goto(`http://localhost:4173${path}`, { waitUntil: 'domcontentloaded', timeout: 15000 });
    await page.waitForTimeout(900);
    await page.screenshot({ path: `/tmp/i18ntool/${name}-${lng}.png` });
    const txt = (await page.locator('main').innerText().catch(() => '')).slice(0,130).replace(/\s+/g,' ');
    console.log(`${lng} ${path.padEnd(19)} ${errs.length ? 'ERR '+errs.join(';') : 'ok'} :: ${txt}`);
    errs.length = 0;
  }
  await ctx.close();
}
