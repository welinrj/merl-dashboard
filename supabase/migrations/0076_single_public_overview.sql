-- Consolidate the anonymous portal into one public overview.
-- Project profiles and recorded implementation coverage are public catalogue
-- information. Results, beneficiaries and expenditure remain limited to
-- reporting periods that the DoCC M&E reviewer has approved.

ALTER TABLE public.public_portal_projects
  ADD COLUMN IF NOT EXISTS project_manager text,
  ADD COLUMN IF NOT EXISTS cumulative_expenditure_vuv numeric,
  ADD COLUMN IF NOT EXISTS utilisation_pct numeric;

ALTER TABLE public.public_portal_summary
  ADD COLUMN IF NOT EXISTS total_utilised_vuv numeric,
  ADD COLUMN IF NOT EXISTS financial_utilisation_pct numeric;
...
BEGIN
  ...
  INSERT INTO public.public_portal_projects(
    ...
    docc_themes,project_manager,cumulative_expenditure_vuv,utilisation_pct
  )
...
DO $$ DECLARE tbl text; BEGIN
  FOREACH tbl IN ARRAY ARRAY[
    'projects','indicator_progress','reporting_periods','beneficiaries','financial_progress',
    'project_area_councils','dashboard_kpi_config','project_indicators',
    'project_organizations','organizations','users'
  ] LOOP
